...
export const itineraryApi = {
  ...
  getCulturalTips: async (destination) => {
    const response = await api.get(
      `/cultural-tips/${encodeURIComponent(destination)}`,
    );
    return response.data;
  },
};
...
export const hiddenGemsApi = {
  getByCity: async (city, limit = 20) => {
    const response = await api.get(
      `/hidden-gems/${encodeURIComponent(city)}?limit=${limit}`,
    );
    return response.data;
  },
...
export const chatApi = {
  ...
  quickAction: async (action, tripId) => {
    const response = await api.post(
      `/chat/quick-action?action=${action}&trip_id=${tripId}`,
    );
    return response.data;
  },
...
export const metaApi = {
  get: async (docId) => {
    const response = await api.get(`/meta/${docId}`);
    return response.data;
  },
};
...
export const emergencyApi = {
  triggerSOS: async (data) => {
    const response = await api.post("/emergency/trigger-sos", data);
    return response.data;
  },
};
